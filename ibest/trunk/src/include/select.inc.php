<?
/* $Id: select.inc.php,v 1.1.1.1 2003/03/29 19:55:21 binary Exp $ */
...
function gera_select_from_result($result, $selected, $params, $permite_null=0)
{
    if (!is_array($selected))
        $selected = array($selected);
...
function gera_select_g($sql, $value, $texto, $tabela, $selected="", $params="", $permite_null=0)
{
    $query = "
        SELECT
    ...
    gera_select_from_result($sql->query($query), $selected, $params, $permite_null);
}
...
function gera_select_data($nome, $array_data=array(), $ano_inicio = "", $ano_fim = "", $permite_null="")
{
    ...
    if (sizeof($array_data) < 2)
        $array_data = array("dia" => date("d",time()), "mes" => date("m",time()), "ano" => date("Y",time()));
