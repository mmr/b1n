...
extract_request_var("cgv_nome",             $dados["cgv_nome"]);
extract_request_var("cgv_desc",             $dados["cgv_desc"]);
$dados = trim_r($dados);
...
{
case "inserir":
    if (! tem_permissao(FUNC_CAD_CARGO_GV_INSERIR))
    {
    ...
    if ($acao == "go")
    {
        ...
        if(!sizeof($error_msgs))
        {
            ...
            {
                log_fnc($sql, FUNC_CAD_CARGO_GV_INSERIR, $dados["id"]);
                if (! tem_permissao(FUNC_CAD_CARGO_GV_LISTAR)) 
                {
                    include(ACESSO_NEGADO);
                    break;
                }
                log_fnc($sql, FUNC_CAD_CARGO_GV_LISTAR);
    ...
    break;
case "alterar":
    if (! tem_permissao(FUNC_CAD_CARGO_GV_ALTERAR))
    {
        include(ACESSO_NEGADO);
        break;
    }
    if ($acao == "go")
    {
        ...
        if(!sizeof($error_msgs))
        {
            ...
            {
                log_fnc($sql, FUNC_CAD_CARGO_GV_ALTERAR, $dados["id"]);
                if (! tem_permissao(FUNC_CAD_CARGO_GV_LISTAR))
                {
                    include(ACESSO_NEGADO);
                    break;
    ...
    break;
case "apagar":
    if (! tem_permissao(FUNC_CAD_CARGO_GV_APAGAR))
    {
        include(ACESSO_NEGADO);
        break;
    }
    if ($acao == "go")
    {
        ...
        {
            log_fnc($sql, FUNC_CAD_CARGO_GV_APAGAR, $dados["id"]);
            if (! tem_permissao(FUNC_CAD_CARGO_GV_LISTAR))
            {
                include(ACESSO_NEGADO);
                break;
    ...
    break;
case "consultar":
    if (! tem_permissao(FUNC_CAD_CARGO_GV_CONSULTAR))
    {
        include(ACESSO_NEGADO);
        break;
    }
    ...
    log_fnc($sql, FUNC_CAD_CARGO_GV_CONSULTAR, $dados["id"]);
