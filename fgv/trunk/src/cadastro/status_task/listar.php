<? /* $Id: listar.php,v 1.10 2002/07/31 21:10:47 binary Exp $ */ ?>

<? $list_data = busca_status_task($sql, $busca); ?>
        ...
        <table border='0' cellspacing='1' cellpadding='5' width='100%' class='text'>
          <tr>
            <td class="textwhitemini" colspan="3" bgcolor="#336699" height="17">
              <img src="images/icone.gif" width="23" height="17" align="absbottom">&nbsp;&nbsp;Status de task - Busca
            </td>
          </tr>
          <tr>
        ...
        <table border="0" cellspacing="1" cellpadding="5" width="100%" class="text">
          <tr>
            <td class="textwhitemini" COLSPAN="4" bgColor="#336699" HEIGHT="17">
              <img src="images/icone.gif" width="23" height="17" align="absbottom">&nbsp;&nbsp;Status de task
            </td>
          </tr>
<?
...
?>
          <tr>
            <td bgcolor='#ffffff'>&nbsp;<?= in_html($item["stt_nome"]) ?></td>
            <td bgcolor='#ffffff'><a href="<?= $_SERVER["SCRIPT_NAME"] ?>?suppagina=<?= $suppagina ?>&pagina=<?= $pagina ?>&subpagina=consultar&id=<?= $item["stt_id"] ?>">Consultar</a></td>
            <?
            if( $item["stt_nome"] == "Arquivada" ||
                $item["stt_nome"] == "Em aberto" ||
                $item["stt_nome"] == "Baixada" )
            {
            ?>
            ...
            {
            ?>
                <td bgcolor='#ffffff'><a href="<?= $_SERVER["SCRIPT_NAME"] ?>?suppagina=<?= $suppagina ?>&pagina=<?= $pagina ?>&subpagina=alterar&id=<?= $item["stt_id"] ?>">Editar</a></td>
                <td bgcolor='#ffffff'><a href="<?= $_SERVER["SCRIPT_NAME"] ?>?suppagina=<?= $suppagina ?>&pagina=<?= $pagina ?>&subpagina=apagar&id=<?= $item["stt_id"] ?>">Excluir</a></td>
            <?
            }
            ?>
          </tr>
