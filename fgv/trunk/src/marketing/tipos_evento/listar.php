<? /* $Id: listar.php,v 1.7 2002/07/31 21:10:47 binary Exp $ */ ?>

<? $list_data = busca_tipo_evento($sql, $busca); ?>
...
<?
    ...
    ?>
        <tr>
          <td bgcolor='#ffffff'>&nbsp;<?= in_html($item["tev_nome"]) ?></td>
          <td bgcolor='#ffffff'><a href="<?= $_SERVER["SCRIPT_NAME"] ?>?suppagina=<?= $suppagina ?>&pagina=<?= $pagina ?>&subpagina=consultar&id=<?= $item["tev_id"] ?>">Consultar</a></td>
        <?
        if( $item["tev_nome"] == "Café & Negócios" ||
            $item["tev_nome"] == "Prêmio Gestão"   ||
            $item["tev_nome"] == "SuperAção" )
        {
        ?>
        ...
        {
            ?>
                <td bgcolor='#ffffff'><a href="<?= $_SERVER["SCRIPT_NAME"] ?>?suppagina=<?= $suppagina ?>&pagina=<?= $pagina ?>&subpagina=alterar&id=<?= $item["tev_id"] ?>">Editar</a></td>
                <td bgcolor='#ffffff'><a href="<?= $_SERVER["SCRIPT_NAME"] ?>?suppagina=<?= $suppagina ?>&pagina=<?= $pagina ?>&subpagina=apagar&id=<?= $item["tev_id"] ?>">Excluir</a></td>
            <?
        }
        ?>
       </tr>
