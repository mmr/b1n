    ...
    extends AbstractIdeSupportMojo
{
    ...
    private List forcedArtifact;

    /**
     * @parameter
     */
    private String[] forcedArtifacts;
    ...
    protected boolean setup()
    {
        ...
        {
            for ( int i = 0; i < forcedArtifacts.length; i++ )
            {
                forcedArtifact.add( forcedArtifacts[i] );
            }
        }
        ...
        throws MojoExecutionException
    {
        ...
        for ( int i = 0; i < dependencies.length; i++ )
        {
            ...
            if ( !forcedArtifact.contains(dependency.getArtifactId()) && (dependency.isProvided() || dependency.isTestDependency()))
            {
                System.out.println("IGNORADO: " + dependency.getArtifactId());
